...
def rank_iteration(data1, data2):
    ...
    fit1 = mirt.mirt(data1, itemtype='2PL')
    fit2 = mirt.mirt(data2, itemtype='2PL')
...
if __name__ =='__main__':
    ...
    self_data_gpt4 = self_data[self_data.user_id.isin(user_ids)][cols].set_index('user_id')

    # Binarize scores for 2PL (0 vs >0)
    gpt4_data = (gpt4_data > 0).astype(int)
    self_data_gpt4 = (self_data_gpt4 > 0).astype(int)

    data1_symp_rank, data2_symp_rank = rank_significance(gpt4_data, self_data_gpt4, list(user_ids), num_permutations=500)
    combined_data_symp_rank = {'gpt4': data1_symp_rank, 'self_report': data2_symp_rank}

    with open('/cronus_data/avirinchipur/reasoning_for_psych/irt/gpt4_sr_fulldata_ranking_2PL.pkl', 'wb') as f:
        pickle.dump(combined_data_symp_rank, f, pickle.HIGHEST_PROTOCOL)

    #to r
    pandas2ri.activate()
    gpt4_sample = pandas2ri.py2rpy(gpt4_data)
    sr_sample_gpt4 = pandas2ri.py2rpy(self_data_gpt4)
    pandas2ri.deactivate()
    gpt4_ranks, sr_ranks_gpt4 = rank_iteration(gpt4_sample, sr_sample_gpt4)
    fulldata_ranks = {'gpt4': gpt4_ranks, 'self_report': sr_ranks_gpt4, 'columns': cols[1:]}

    pd.DataFrame(fulldata_ranks).to_csv('/cronus_data/avirinchipur/reasoning_for_psych/irt/gpt4_sr_fulldata_ranking_2PL.csv',
                                        index=False)
    ...
    self_data_gpt5 = self_data[self_data.user_id.isin(user_ids)][cols].set_index('user_id')

    # Binarize scores for 2PL (0 vs >0)
    gpt5_data = (gpt5_data > 0).astype(int)
    self_data_gpt5 = (self_data_gpt5 > 0).astype(int)

    data1_symp_rank, data2_symp_rank = rank_significance(gpt5_data, self_data_gpt5, list(user_ids), num_permutations=500)
    combined_data_symp_rank = {'gpt5': data1_symp_rank, 'self_report': data2_symp_rank}

    with open('/cronus_data/avirinchipur/reasoning_for_psych/irt/gpt5_sr_fulldata_ranking_2PL.pkl', 'wb') as f:
        pickle.dump(combined_data_symp_rank, f, pickle.HIGHEST_PROTOCOL)

    #to r
    pandas2ri.activate()
    gpt5_sample = pandas2ri.py2rpy(gpt5_data)
    sr_sample_gpt5 = pandas2ri.py2rpy(self_data_gpt5)
    pandas2ri.deactivate()
    gpt5_ranks, sr_ranks_gpt5 = rank_iteration(gpt5_sample, sr_sample_gpt5)
    fulldata_ranks = {'gpt5': gpt5_ranks, 'self_report': sr_ranks_gpt5, 'columns': cols[1:]}

    pd.DataFrame(fulldata_ranks).to_csv('/cronus_data/avirinchipur/reasoning_for_psych/irt/gpt5_sr_fulldata_ranking_2PL.csv', index=False)
